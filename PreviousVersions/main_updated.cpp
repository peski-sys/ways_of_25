...
struct Watch {
    ...
    // Pre-built Naasa order headers (copy of g_naasa_order_hdrs at watch creation).
...
// ── Order placement ───────────────────────────────────────────────────────────
// Build Naasa order JSON payload. All fields match the browser POST from 12.order.
static std::string build_naasa_order_payload(const std::string& scrip, int qty, double price) {
    char price_buf[32];
    snprintf(price_buf, sizeof(price_buf), "%.1f", price);
    std::string s;
    s.reserve(512);
    s  = "{\"TradingAccount\":\"CNC\",\"Exchange\":\"NEPSE\",";
    s += "\"Scrip\":\"" + scrip + "\",";
    s += "\"Quantity\":\"" + std::to_string(qty) + "\",";
    s += "\"Price\":\"" + std::string(price_buf) + "\",";
    s += "\"Market\":\"0\",\"OrderTerms\":\"DAY\",";
    s += "\"BuySellIndicator\":\"B\",\"BuySellType\":\"Buy\",";
    s += "\"DeliveryTerms\":\"D\",\"MarketSegment\":\"RL\",";
    s += "\"OrderCategory\":\"NORMAL\",\"OrderType\":\"NORMAL\",";
    s += "\"AccRefCode\":\"SELF\",\"TermValidity\":\"\",";
    s += "\"ProductType\":\"CASH\",\"DisclosedQuantity\":\"\",";
    s += "\"isSquareOff\":0}";
    return s;
}
...
// Naasa order success: HTTP 200 AND ErrorCode == 0 in the JSON body.
static bool is_order_success(long http_code, const std::string& body) {
    if (http_code != 200) return false;
    try {
        json resp = json::parse(body);
        return resp.value("ErrorCode", -1) == 0;
    } catch (...) {}
    return false;
}

// Place one order on watch->order_curl. Measures latency.
// No session-refresh retry — Naasa has no rotating tokens on the order path.
static void place_order(double price, int qty,
    ...
    std::string payload = build_naasa_order_payload(watch->symbol, qty, price);

    // Build the Cookie header value from memory — mirrors main.py's requests.Session exactly.
    // The cookie is prepended to a per-request slist alongside the static headers so that
    // it goes through the exact same path as every other header (CURLOPT_HTTPHEADER only,
    // no cookie engine involved).
    std::string cookie_hdr;
    {
        std::lock_guard<std::mutex> lk(g_naasa_session_mutex);
        cookie_hdr = "Cookie: .AspNetCore.Session=" + g_naasa_asp_cookie;
    }
    auto do_post = [&]() -> HttpResponse {
        ...
        // Build per-request header list: Cookie first, then all static headers.
        ...
        for (curl_slist* p = g_naasa_order_hdrs; p; p = p->next)
            req_hdrs = curl_slist_append(req_hdrs, p->data);

        // No curl_easy_reset() — permanent options survive, HTTP2 connection reused.
        // Only the 6 fields that change per-request are set here.
        curl_easy_setopt(watch->order_curl, CURLOPT_URL,            NAASA_ORDER_URL.c_str());
    ...
    // Order failed — log the Message from the Naasa response JSON
    std::string msg;
    try {
        json resp = json::parse(r.body);
        msg = resp.value("Message", r.body.substr(0, 300));
    ...
    case LWS_CALLBACK_CLIENT_RECEIVE: {
        ...
        while (!sv.empty()) {
            ...
            if (is_first || (prev_ltp >= 0 && prev_ltp != ltp)) {
                place_order(bid_price, qty, ctx->watch_id, t_recv);
            }
        }
        break;
    }
...
static void heartbeat_loop() {
    ...
    // Continuous order_curl warmup: HEAD NAASA_ORDER_URL every 45s on all active watches.
    ...
    while (!g_hb_stop.load()) {
        ...
        // Continuous order_curl keepalive: GET /Home/ServerTime every 5s — mirrors Rust
        // task_every_5s. Same host:port as order URL → HTTP/2 connection never idles out.
        if (std::chrono::duration_cast<std::chrono::seconds>(
            ...
            for (auto& w : snaps) {
                ...
                {
                    std::lock_guard<std::mutex> sl(g_naasa_session_mutex);
                    w_ck = "Cookie: .AspNetCore.Session=" + g_naasa_asp_cookie;
                }
                curl_slist* wh = nullptr;
                wh = curl_slist_append(wh, w_ck.c_str());
                wh = curl_slist_append(wh, "Accept: application/json, text/javascript, */*; q=0.01");
                wh = curl_slist_append(wh, "X-Requested-With: XMLHttpRequest");
                std::string body;
                // No reset — only switch method+URL, permanent options intact.
                curl_easy_setopt(w->order_curl, CURLOPT_URL,        NAASA_SERVER_TIME_URL.c_str());
            ...
            if (!warmup_done && nsecs >= 10 * 3600 + 59 * 60 + 55 && nsecs < 11 * 3600) {
                ...
                for (auto& wid : active_wids) {
                    ...
                    if (!w_ptr || !w_ptr->order_curl) continue;
                    {
                        ...
                        {
                            std::lock_guard<std::mutex> sl(g_naasa_session_mutex);
                            w_ck2 = "Cookie: .AspNetCore.Session=" + g_naasa_asp_cookie;
                        }
                        curl_slist* wh2 = nullptr;
                        wh2 = curl_slist_append(wh2, w_ck2.c_str());
                        wh2 = curl_slist_append(wh2, "Accept: application/json, text/javascript, */*; q=0.01");
                        // No reset — only switch method+URL, permanent options intact.
                        curl_easy_setopt(w_ptr->order_curl, CURLOPT_URL,        NAASA_SERVER_TIME_URL.c_str());
...
static void cmd_add_watch() {
    ...
    // Per-watch order handle: independent curl handle for Naasa order POSTs.
    // Loads cookies from NAASA_COOKIE_FILE (set by Keycloak login) on each reset.
    ...
    // No curl_easy_reset() is ever called on order_curl after this point.
    ...
    // Initial warmup: establish TCP+TLS+HTTP2 immediately.
    // Only dynamic fields (URL, method, headers, writedata) set here.
    {
        std::string w_ck;
        {
            std::lock_guard<std::mutex> sl(g_naasa_session_mutex);
        ...
        wh = curl_slist_append(wh, "X-Requested-With: XMLHttpRequest");
        std::string dummy;
        curl_easy_setopt(watch->order_curl, CURLOPT_URL,        NAASA_SERVER_TIME_URL.c_str());
    ...
    // Copy static Naasa order headers into per-watch list.
    for (curl_slist* p = g_naasa_order_hdrs; p; p = p->next)
        watch->order_hdrs = curl_slist_append(watch->order_hdrs, p->data);
...
static void cmd_fire(const std::string& sym) {
    ...
    // 3. Snapshot cookie + build header list once — reused for every shot.
    //    Uses watch->order_curl directly (already warm from 5s heartbeat GET /Home/ServerTime)
    //    instead of a fresh handle, eliminating TCP+TLS+HTTP2 connection setup overhead.
    std::string fire_cookie_hdr;
    {
        std::lock_guard<std::mutex> lk(g_naasa_session_mutex);
        fire_cookie_hdr = "Cookie: .AspNetCore.Session=" + g_naasa_asp_cookie;
    }
    std::string payload = build_naasa_order_payload(watch->symbol, qty, bid);
    curl_slist* hdrs_snap = nullptr;
    hdrs_snap = curl_slist_append(hdrs_snap, fire_cookie_hdr.c_str());
    for (curl_slist* p = g_naasa_order_hdrs; p; p = p->next)
        hdrs_snap = curl_slist_append(hdrs_snap, p->data);
...
int main() {
    ...
    // DNS pin for Naasa — bypass runtime DNS on the hot order path.
